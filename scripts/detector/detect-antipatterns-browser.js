/**
 * Anti-Pattern Browser Detector for Asterframe
...
function checkElementQualityDOM(el) {
  ...
  const rect = el.getBoundingClientRect();
  const lineMax = (typeof window !== 'undefined' && window.__ASTERFRAME_CONFIG__?.lineLengthMax) || 80;
...
function checkTypography() {
  ...
  for (const el of document.querySelectorAll('p, h1, h2, h3, h4, h5, h6, li, td, th, dd, blockquote, figcaption, a, button, label, span')) {
    // Skip asterframe's own elements
    if (el.closest && el.closest('.asterframe-overlay, .asterframe-label, .asterframe-banner, .asterframe-tooltip')) continue;
...
if (IS_BROWSER) {
  ...
  const EXTENSION_MODE = (_myScript && _myScript.dataset.asterframeExtension === 'true')
    || document.documentElement.dataset.asterframeExtension === 'true';
  ...
  styleEl.textContent = `
    @keyframes asterframe-reveal {
      from { opacity: 0; }
      to { opacity: 1; }
    }
    .asterframe-overlay:not(.asterframe-banner) {
      pointer-events: none;
      outline: 2px solid ${OUTLINE_COLOR};
      border-radius: 4px;
      transition: outline-color 0.15s ease;
      animation: asterframe-reveal 0.4s cubic-bezier(0.16, 1, 0.3, 1) both;
      animation-play-state: paused;
      border-top-left-radius: 0;
    }
    .asterframe-overlay.asterframe-visible {
      animation-play-state: running;
    }
    .asterframe-overlay.asterframe-hover {
      outline-color: ${BRAND_COLOR_HOVER};
      z-index: 100001 !important;
    }
    .asterframe-overlay.asterframe-hover .asterframe-label {
      background: ${BRAND_COLOR_HOVER};
    }
    .asterframe-overlay.asterframe-spotlight {
      z-index: 100002 !important;
    }
    .asterframe-overlay.asterframe-spotlight-dimmed {
      opacity: 0.15 !important;
      animation: none !important;
      filter: blur(3px);
    }
    .asterframe-spotlight-backdrop {
      position: fixed;
    ...
    .asterframe-spotlight-backdrop.asterframe-visible {
      opacity: 1;
    }
    .asterframe-hidden .asterframe-overlay${EXTENSION_MODE ? '' : ':not(.asterframe-banner)'} {
  ...
  function getSpotlightBackdrop() {
    if (!spotlightBackdrop) {
      ...
      spotlightBackdrop.className = 'asterframe-spotlight-backdrop';
  ...
  function showSpotlight(target) {
    ...
    if (window.__ASTERFRAME_CONFIG__?.spotlightBlur === false) {
    ...
    bd.classList.add('asterframe-visible');
  }

  function hideSpotlight() {
    spotlightTarget = null;
    if (spotlightBackdrop) spotlightBackdrop.classList.remove('asterframe-visible');
  }
  ...
  function repositionOverlays() {
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('asterframe-banner')) continue;
  ...
  const visibilityObserver = new IntersectionObserver((entries) => {
    for (const entry of entries) {
      const overlay = entry.target._asterframeOverlay;
      if (!overlay) continue;
      if (entry.isIntersecting) {
        ...
        if (!overlay._revealed) {
          ...
          requestAnimationFrame(() => {
            overlay.classList.add('asterframe-visible');
  ...
  function detachOverlay(overlay) {
    ...
    if (overlay._targetEl && overlay._targetEl._asterframeOverlay === overlay) {
      visibilityObserver.unobserve(overlay._targetEl);
      delete overlay._targetEl._asterframeOverlay;
    }
  ...
  document.addEventListener('transitionend', (e) => {
    ...
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('asterframe-banner') || o.style.display === 'none') continue;
  ...
  const highlight = function(el, findings) {
    ...
    label.className = 'asterframe-label';
    ...
    const onMouseEnter = () => {
      isHovered = true;
      outline.classList.add('asterframe-hover');
    ...
    const onMouseLeave = () => {
      isHovered = false;
      outline.classList.remove('asterframe-hover');
  ...
  const showPageBanner = function(findings) {
    ...
    banner.className = 'asterframe-overlay asterframe-banner';
    ...
    if (!EXTENSION_MODE) {
      ...
      toggle.addEventListener('click', () => {
        ...
        document.body.classList.toggle('asterframe-hidden', !overlaysVisible);
  ...
  function buildSelectorSegment(el) {
    ...
    if (el.classList && el.classList.length > 0) {
      const classes = [...el.classList]
        .filter(c => !c.startsWith('asterframe-') && !isLikelyHashedClass(c))
        .slice(0, 2);
      if (classes.length > 0) {
  ...
  function collectVisualContrastCandidates(options = {}) {
    ...
    for (const el of document.querySelectorAll('*')) {
      ...
      if (el.closest('.asterframe-overlay, .asterframe-label, .asterframe-banner, .asterframe-tooltip')) continue;
      if (el.closest('[id^="asterframe-live-"]')) continue;
  ...
  async function sampleVisualBackgroundAtPoint(el, point, textColor, depth = 0) {
    ...
    for (const node of nodes) {
      ...
      if (node.closest?.('.asterframe-overlay, .asterframe-label, .asterframe-banner, .asterframe-tooltip')) continue;
  ...
  const printSummary = function(allFindings) {
    if (allFindings.length === 0) {
      console.log('%c[asterframe] No anti-patterns found.', 'color: #22c55e; font-weight: bold');
      return;
    }
    console.group(
      `%c[asterframe] ${allFindings.length} anti-pattern${allFindings.length === 1 ? '' : 's'} found`,
  ...
  function browserDesignSystemConfig() {
    const raw = window.__ASTERFRAME_CONFIG__?.designSystem;
  ...
  function collectBrowserFindings() {
    ...
    const _disabled = EXTENSION_MODE ? (window.__ASTERFRAME_CONFIG__?.disabledRules || []) : [];
    ...
    for (const el of document.querySelectorAll('*')) {
      // Skip asterframe's own elements and any descendants (overlays, labels, banner, nav buttons)
      if (el.closest('.asterframe-overlay, .asterframe-label, .asterframe-banner, .asterframe-tooltip')) continue;
      // Skip browser extension elements (Claude, etc.)
      const elId = el.id || '';
      if (elId.startsWith('claude-') || elId.startsWith('cic-')) continue;
      // Skip the asterframe live-mode overlay (highlight, tooltip, bar, picker, toast).
    ...
    // Clone the document and strip asterframe-live overlay nodes before the
    ...
    for (const node of docClone.querySelectorAll('[id^="asterframe-live-"]')) {
      node.remove();
    }
  ...
  function shouldRunVisualContrast(options = {}) {
    return options.visualContrast === true || window.__ASTERFRAME_CONFIG__?.visualContrast === true;
  }

  function visualContrastOptions(options = {}) {
    const config = window.__ASTERFRAME_CONFIG__ || {};
  ...
  function postExtensionError(err) {
    if (!EXTENSION_MODE) return;
    window.postMessage({
      source: 'asterframe-error',
      message: err?.message || String(err),
    }, '*');
  }

  function reportVisualContrastError(err, detail = {}) {
    window.dispatchEvent(new CustomEvent('asterframe-visual-contrast-error', {
      detail: {
        ...detail,
        message: err?.message || String(err),
      },
    ...
    if (EXTENSION_MODE) {
      ...
      console.warn('[asterframe] visual contrast scan failed', err);
    }
  }
        ...
        waitForVisualPaint()
          ...
          .then(result => {
            ...
            if (added) {
              ...
              window.dispatchEvent(new CustomEvent('asterframe-visual-contrast-resolved', {
                detail: {
  ...
  if (EXTENSION_MODE) {
    ...
    window.addEventListener('message', (e) => {
      if (e.source !== window || !e.data || e.data.source !== 'asterframe-command') return;
      if (e.data.action === 'scan') {
        if (e.data.config) window.__ASTERFRAME_CONFIG__ = e.data.config;
        try {
          scan(e.data.config || {});
        } catch (err) {
          postExtensionError(err);
      ...
      if (e.data.action === 'toggle-overlays') {
        const visible = !document.body.classList.contains('asterframe-hidden');
        document.body.classList.toggle('asterframe-hidden', visible);
        window.postMessage({ source: 'asterframe-overlays-toggled', visible: !visible }, '*');
      }
      if (e.data.action === 'remove') {
        clearOverlays();
        styleEl.remove();
        if (spotlightBackdrop) { spotlightBackdrop.remove(); spotlightBackdrop = null; }
        document.body.classList.remove('asterframe-hidden');
      }
      if (e.data.action === 'highlight') {
        try {
          ...
          if (target) {
            ...
            for (const o of overlays) {
              if (o.classList.contains('asterframe-banner')) continue;
              const isMatch = o._targetEl === target;
              o.classList.toggle('asterframe-spotlight', isMatch);
              o.classList.toggle('asterframe-spotlight-dimmed', !isMatch);
              if (isMatch) {
                // Force the matching overlay visible immediately, don't wait for IntersectionObserver
                o.style.display = '';
                o.style.animation = 'none';
                o.classList.add('asterframe-visible');
                o._revealed = true;
                positionOverlay(o);
              }
            }
      ...
      if (e.data.action === 'unhighlight') {
        hideSpotlight();
        for (const o of overlays) {
          o.classList.remove('asterframe-spotlight');
          o.classList.remove('asterframe-spotlight-dimmed');
        }
      }
    });
    window.postMessage({ source: 'asterframe-ready' }, '*');
  } else {
    if (window.__ASTERFRAME_CONFIG__?.autoScan !== false) {
      const runAutoScan = () => {
        try {
          scan();
        } catch (err) {
          console.warn('[asterframe] scan failed', err);
        }
      };
  ...
  window.asterframeDetect = detect;
  window.asterframeDetectAsync = detectAsync;
  window.asterframeScan = scan;
  window.asterframeScanAsync = scanAsync;
  window.asterframeCollectVisualContrastCandidates = collectVisualContrastCandidates;
  window.asterframeAnalyzeVisualContrast = analyzeVisualContrast;
  window.asterframeGetLastVisualContrastAnalyses = () => lastVisualContrastAnalyses.slice();
}
