#!/usr/bin/env node
/**
 * `/asterframe hooks <on|off|status|reset>` — manage the design hook runtime
 * via the `hook` key and shared detector ignores via the `detector` key in
 * .asterframe/config.json / .asterframe/config.local.json.
 *
 * Usage:
 ...
 *
 * Designed to be invoked by the LLM from the references/hooks.md flow.
...
const ASTERFRAME_HOOK_COMMAND_MARKERS = [
  'skills/asterframe/scripts/hook-probe.mjs',
  'skills/asterframe/scripts/hook.mjs',
  'skills/asterframe/scripts/hook-before-edit.mjs',
  'skills/asterframe/scripts/hook-after-edit.mjs',
  'skills/asterframe/scripts/hook-stop.mjs',
];
...
const HOOK_MANIFEST_TARGETS = [
  {
    provider: '.claude',
    skillRel: '.claude/skills/asterframe',
    destRel: '.claude/settings.local.json',
    sharedDestRel: '.claude/settings.json',
    manifest: () => ({
      description: 'Asterframe design detector: runs after Edit/Write/MultiEdit on UI files and surfaces findings as system reminders.',
      hooks: {
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: 'command',
                command: 'node "${CLAUDE_PROJECT_DIR}/.claude/skills/asterframe/scripts/hook.mjs"',
  ...
  {
    provider: '.agents',
    skillRel: '.agents/skills/asterframe',
    destRel: '.codex/hooks.json',
    manifest: () => ({
      description: 'Asterframe design detector: runs after Edit/Write/apply_patch on UI files and surfaces findings as system reminders.',
      hooks: {
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: 'command',
                command: 'node "$(git rev-parse --show-toplevel)/.agents/skills/asterframe/scripts/hook.mjs"',
                timeout: TIMEOUT_SECONDS,
                statusMessage: STATUS_MESSAGE,
              },
            ],
  ...
  {
    provider: '.cursor',
    skillRel: '.cursor/skills/asterframe',
    ...
    manifest: () => ({
      ...
      hooks: {
        preToolUse: [
          {
            command: 'node ".cursor/skills/asterframe/scripts/hook-before-edit.mjs"',
            timeout: TIMEOUT_SECONDS,
          },
        ],
      },
...
function writeHookConfig(cwd, hookConfig, opts = {}) {
  ...
  // (consent, quiet, auditLog) survive a `/asterframe hooks` edit.
...
function statusReport(cwd) {
  ...
  const envKill = process.env.ASTERFRAME_HOOK_DISABLED;
  const envState = envKill ? `ASTERFRAME_HOOK_DISABLED=${envKill}` : 'unset';
  const cfgPath = path.relative(cwd, getConfigPath(cwd)) || '.asterframe/config.json';
  const localPath = path.relative(cwd, getLocalConfigPath(cwd)) || '.asterframe/config.local.json';
  const cachePath = path.relative(cwd, getCachePath(cwd)) || '.asterframe/hook.cache.json';
...
function repairHookManifests(cwd) {
  ...
  for (const target of HOOK_MANIFEST_TARGETS) {
    ...
    if (sharedDest && fileHasAsterframeHookMarker(sharedDest)) {
      pruneAsterframeHookFromManifest(dest);
...
function mergeHookManifests(existing, fresh) {
  ...
  for (const event of hookEvents) {
    const preserved = stripAsterframeHookEntries(existingHooks[event]);
...
function fileHasAsterframeHookMarker(filePath) {
  ...
  return valueHasAsterframeHookMarker(parsed.hooks);
}

function valueHasAsterframeHookMarker(value) {
  if (typeof value === 'string') {
    return ASTERFRAME_HOOK_COMMAND_MARKERS.some((marker) => value.includes(marker));
  }
  if (Array.isArray(value)) return value.some(valueHasAsterframeHookMarker);
  if (value && typeof value === 'object') return Object.values(value).some(valueHasAsterframeHookMarker);
  return false;
}

function stripAsterframeHookEntry(entry) {
  if (!entry || typeof entry !== 'object') return entry;
  if (valueHasAsterframeHookMarker(entry.command) || valueHasAsterframeHookMarker(entry.args)) {
    return null;
  }
  if (!Array.isArray(entry.hooks)) return entry;

  const strippedHooks = entry.hooks
    .map(stripAsterframeHookEntry)
    .filter(Boolean);

  if (strippedHooks.length === 0 && entry.hooks.some(valueHasAsterframeHookMarker)) {
    return null;
  }
  return { ...entry, hooks: strippedHooks };
}

function stripAsterframeHookEntries(entries) {
...
function pruneAsterframeHookFromManifest(manifestPath) {
  if (!fileHasAsterframeHookMarker(manifestPath)) return false;
  let parsed;
  try {
  ...
  for (const [event, entries] of Object.entries(existingHooks)) {
    const kept = stripAsterframeHookEntries(entries);
...
function addIgnoreRule(cwd, args) {
  ...
  if (!rule) throw new Error('Pass a rule id, e.g. /asterframe hooks ignore-rule side-tab');
  if (rule === 'overused-font' && !parsed.allValues) {
    throw new Error('overused-font is value-specific by default. Use /asterframe hooks ignore-value overused-font <font> for a confirmed font, or /asterframe hooks ignore-rule overused-font --all-values only when the user asked to ignore overused fonts generally.');
  }
...
function addIgnoreFile(cwd, glob) {
  if (!glob) throw new Error('Pass a glob, e.g. /asterframe hooks ignore-file "src/legacy/**"');
  const config = mergeDetectorConfig(readRawDetectorConfig(cwd));
...
function addIgnoreValue(cwd, args) {
  ...
  if (!parsed.rule || !parsed.value) {
    throw new Error('Pass a rule id and value, e.g. /asterframe hooks ignore-value overused-font Inter');
  }
