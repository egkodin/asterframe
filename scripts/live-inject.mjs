 ...
 *
 * On first live run, the agent generates `.asterframe/live/config.json`
...
import { resolveLiveConfigPath } from './lib/asterframe-paths.mjs';
import {
...
const MARKER_OPEN_TEXT = 'asterframe-live-start';
const MARKER_CLOSE_TEXT = 'asterframe-live-end';
const IGNORE_MARKER_OPEN = '# asterframe-live-ignore-start';
const IGNORE_MARKER_CLOSE = '# asterframe-live-ignore-end';
...
export const LIVE_IGNORE_PATTERNS = Object.freeze([
  ...
  '.asterframe/live/server.json',
  '.asterframe/live/sessions/',
  '.asterframe/live/previews/',
  '.asterframe/live/annotations/',
  '.asterframe/live/cache/',
  '.asterframe/live/manual-edit-apply-transaction.json',
  '.asterframe/live/manual-edit-events.jsonl',
  '.asterframe/live/manual-edit-evidence/',
  '.asterframe/live/pending-manual-edits.json',
  '.asterframe/live/deferred-svelte-component-accepts.json',
  '.asterframe-live.json',
  '.asterframe-live/',
  'node_modules/.asterframe-live/',
  'src/lib/asterframe/AsterframeLiveRoot.svelte',
  'src/lib/asterframe/__runtime.js',
  'src/lib/asterframe/[0-9a-f]*/',
]);
...
Reads configuration from .asterframe/live/config.json.

Modes:
  --port PORT   Insert script tag pointing at http://localhost:PORT/live.js
  --remove      Remove the script tag (if present)
  --check       Print whether .asterframe/live/config.json exists and its content
...
function removeTag(content, _syntax) {
  const patterns = [
    /([ \t]*)<!--\s*asterframe-live-start\s*-->[\s\S]*?<!--\s*asterframe-live-end\s*-->([ \t]*(?:\r\n|\n|\r|$)?)/,
    /([ \t]*)\{\/\*\s*asterframe-live-start\s*\*\/\}[\s\S]*?\{\/\*\s*asterframe-live-end\s*\*\/\}([ \t]*(?:\r\n|\n|\r|$)?)/,
  ];
  for (const pat of patterns) {
    let changed = false;
...
// and stash the original `content` value in a `data-asterframe-csp-original`
...
const CSP_MARKER_ATTR = 'data-asterframe-csp-original';
