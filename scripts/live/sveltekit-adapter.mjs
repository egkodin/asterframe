...
export const SVELTE_LIVE_ROOT_COMPONENT = 'src/lib/asterframe/AsterframeLiveRoot.svelte';
export const SVELTE_LAYOUT_MARKER_OPEN = '<!-- asterframe-live-svelte-start -->';
export const SVELTE_LAYOUT_MARKER_CLOSE = '<!-- asterframe-live-svelte-end -->';
export const SVELTE_ROOT_IMPORT = "import AsterframeLiveRoot from '$lib/asterframe/AsterframeLiveRoot.svelte';";
...
export function buildSvelteLiveRootComponent(port) {
  ...
  const HOST_ID = 'asterframe-live-root';

  onMount(() => {
    let host = document.querySelector('asterframe-live-root#' + HOST_ID) || document.getElementById(HOST_ID);
    if (!host) {
      host = document.createElement('asterframe-live-root');
      host.id = HOST_ID;
      document.body.appendChild(host);
    }

    host.dataset.asterframeLiveAdapter = 'sveltekit';
    ...
    if (!root.querySelector('style[data-asterframe-live-reset]')) {
      const reset = document.createElement('style');
      reset.dataset.asterframeLiveReset = 'true';
      reset.textContent = ':host, :host *, * { box-sizing: border-box; }';
      root.appendChild(reset);
    }

    window.__ASTERFRAME_LIVE_ADAPTER__ = 'sveltekit';
    window.__ASTERFRAME_LIVE_UI_ROOT__ = root;
    window.__ASTERFRAME_LIVE_CHROME_MOUNT__ = {
      adapter: 'sveltekit',
      version: 1,
      host,
      root,
    ...
    script.src = LIVE_URL;
    script.async = true;
    script.dataset.asterframeLiveScript = 'true';
    document.head.appendChild(script);

    return () => {
      script.remove();
      if (window.__ASTERFRAME_LIVE_UI_ROOT__ === root) delete window.__ASTERFRAME_LIVE_UI_ROOT__;
      if (window.__ASTERFRAME_LIVE_CHROME_MOUNT__?.root === root) delete window.__ASTERFRAME_LIVE_CHROME_MOUNT__;
      if (window.__ASTERFRAME_LIVE_ADAPTER__ === 'sveltekit') delete window.__ASTERFRAME_LIVE_ADAPTER__;
    };
  });
</script>
`;
