...
export const ASTERFRAME_DIR = '.asterframe';
export const LIVE_DIR = 'live';
export const CRITIQUE_DIR = 'critique';

export function getAsterframeDir(cwd = process.cwd()) {
  return path.join(cwd, ASTERFRAME_DIR);
}

export function getDesignSidecarPath(cwd = process.cwd()) {
  return path.join(getAsterframeDir(cwd), 'design.json');
}
...
export function getLiveDir(cwd = process.cwd()) {
  return path.join(getAsterframeDir(cwd), LIVE_DIR);
}
...
export function resolveLiveConfigPath({ cwd = process.cwd(), scriptsDir, env = process.env } = {}) {
  if (env.ASTERFRAME_LIVE_CONFIG && env.ASTERFRAME_LIVE_CONFIG.trim()) {
    const configured = env.ASTERFRAME_LIVE_CONFIG.trim();
...
export function getLegacyLiveServerPath(cwd = process.cwd()) {
  return path.join(cwd, '.asterframe-live.json');
}
...
export function getLegacyLiveSessionsDir(cwd = process.cwd()) {
  return path.join(cwd, '.asterframe-live', 'sessions');
}
...
export function getCritiqueDir(cwd = process.cwd()) {
  return path.join(getAsterframeDir(cwd), CRITIQUE_DIR);
}

export function getLegacyLiveAnnotationsDir(cwd = process.cwd()) {
  return path.join(cwd, '.asterframe-live', 'annotations');
}
