...
export function assembleLiveBrowserScript({ token, port, vocabulary, parts }) {
  const prelude =
    `window.__ASTERFRAME_TOKEN__ = '${token}';\n` +
    `window.__ASTERFRAME_PORT__ = ${port};\n` +
    ...
    `window.__ASTERFRAME_VOCAB__ = ${JSON.stringify(vocabulary)};\n`;

  const body = parts.map((part) => {
    const file = part.file || path.basename(part.path || '');
    return `// --- asterframe live script part: ${part.name} (${file}) ---\n${part.source}`;
  }).join('\n');
