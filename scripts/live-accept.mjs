...
Output (JSON):
  ...
  if (svelteComponentManifest) {
    ...
    if (result.carbonize) {
      result.todo = 'REQUIRED before next poll: carbonize cleanup in ' + result.file + '. See references/live.md "Required after accept".';
    }
  ...
  if (isDiscard) {
    ...
    // five-step checklist lives in references/live.md (loaded once per
 ...
 * <div data-asterframe-carbonize> so the slot keeps a single root node.
 */
function buildCarbonizeReplacement({
  ...
  const pushCarbonizeBody = (bodyIndent) => {
    ...
    lines.push(bodyIndent + commentSyntax.open + ' asterframe-carbonize-start ' + id + ' ' + commentSyntax.close);
    lines.push(bodyIndent + '<style data-asterframe-css="' + id + '">' + (isJsx ? '{`' : ''));
    ...
    if (paramValues && Object.keys(paramValues).length > 0) {
      lines.push(
        bodyIndent + commentSyntax.open + ' asterframe-param-values ' + id + ': ' + JSON.stringify(paramValues) + ' ' + commentSyntax.close,
      );
    }
    lines.push(bodyIndent + commentSyntax.open + ' asterframe-carbonize-end ' + id + ' ' + commentSyntax.close);
    lines.push(bodyIndent + '<div data-asterframe-variant="' + variantNum + '" ' + variantStyleAttr + '>');
  ...
  if (isJsx) {
    ...
    lines.push(indent + '<div data-asterframe-carbonize="' + id + '" ' + wrapperStyle + '>');
...
function handleAccept(id, variantNum, lines, targetFile, paramValues) {
  ...
  const hasHelperAttrs = variantText.includes('data-asterframe-variant');
...
function readSourceShadowPreviewMeta(content, id) {
  ...
  const wrapperRe = new RegExp('<[^>]+data-asterframe-variants=(["\'])' + escaped + '\\1[^>]*>');
  const match = String(content || '').match(wrapperRe);
  if (!match) return null;
  const tag = match[0];
  if (readHtmlAttr(tag, 'data-asterframe-preview') !== 'source-shadow') return null;
  const sourceFile = readHtmlAttr(tag, 'data-asterframe-source-file');
  const sourceStartLine = Number(readHtmlAttr(tag, 'data-asterframe-source-start'));
  const sourceEndLine = Number(readHtmlAttr(tag, 'data-asterframe-source-end'));
...
function findMarkerBlock(id, lines) {
  let start = -1;
  let end = -1;
  const startPattern = 'asterframe-variants-start ' + id;
  const endPattern = 'asterframe-variants-end ' + id;
 ...
 * the `<div data-asterframe-variants="ID">` outer wrapper so the picked
 ...
 * (with `data-asterframe-variants="ID"`) and its matching `</div>`. We
...
function expandReplaceRange(block, lines, isJsx) {
  ...
  // Walk back for the wrapper `<div data-asterframe-variants="..."` opener.
...
function isVariantEndMarkerLine(line, id) {
  return new RegExp('asterframe-variants-end\\s+' + escapeRegExp(id) + '(?:\\s|--|\\*/|$)').test(line);
}

function hasVariantWrapperAttr(line, id) {
  const escaped = escapeRegExp(id);
  return new RegExp(`data-asterframe-variants\\s*=\\s*(?:"${escaped}"|'${escaped}'|\\{["']${escaped}["']\\})`).test(line);
}
 ...
 *   - CSS `@scope ([data-asterframe-variant="N"])` strings that look like the
...
function extractOriginal(lines, block) {
  ...
  const inner = extractInnerByAttr(text, 'data-asterframe-variant="original"');
...
function extractVariant(lines, block, variantNum) {
  const text = stripStyleAndJoin(lines, block);
  const inner = extractInnerByAttr(text, 'data-asterframe-variant="' + variantNum + '"');
 ...
 *
 * Handles three shapes of `<style data-asterframe-css="ID" ...>`:
...
function extractCss(lines, block, id) {
  const styleAttr = 'data-asterframe-css="' + id + '"';
  let inStyle = false;
  const content = [];
...
function findSessionFile(id, cwd) {
  const marker = 'asterframe-variants-start ' + id;
