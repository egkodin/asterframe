...
// DESIGN sidecar is project-local at .asterframe/design.json, with legacy
...
const DEBUG_MANUAL_EDIT_EVENTS = /^(1|true|yes)$/i.test(process.env.ASTERFRAME_LIVE_DEBUG_EVENTS || '');
...
function loadBrowserScripts() {
  ...
  const detectPaths = [
    ...
    path.join(__dirname, '..', '..', '..', '..', 'cli', 'engine', 'detect-antipatterns-browser.js'),
    path.join(process.cwd(), 'node_modules', 'asterframe', 'cli', 'engine', 'detect-antipatterns-browser.js'),
  ];
  let detectScript = '';
...
function createRequestHandler({ detectScript, liveScriptParts }) {
  return (req, res) => {
    ...
    //   /design-system.json    returns both parsed DESIGN.md and .asterframe/design.json
    ...
    //                          - sidecar: .asterframe/design.json contents when present.
    ...
    if (p === '/design-system.json' || p === '/design-system/raw') {
      ...
      if (jsonStat) {
        try {
          ...
          response.sidecarError = 'Failed to parse .asterframe/design.json: ' + err.message;
        }
      }
...
function sessionFileMetadataFromPollReply(file) {
  ...
  if (!normalized.includes('node_modules/.asterframe-live/') && !normalized.includes('src/lib/asterframe/')) return base;
...
function cleanupSvelteComponentSessionsBeforeExit() {
  try {
    ...
    console.warn('[asterframe] Svelte component session cleanup failed:', err.message);
  }
}
...
function applyLegacyDeferredAcceptsOnStartup() {
  try {
    ...
    if (result.applied > 0 || result.failed > 0) {
      console.log('[asterframe] applied legacy deferred Svelte component accepts:', JSON.stringify(result));
    }
  } catch (err) {
    console.warn('[asterframe] legacy deferred Svelte component accept apply failed:', err.message);
  }
}
...
httpServer.listen(state.port, '127.0.0.1', () => {
  ...
  console.log(`\nAsterframe live server running on ${url}`);
