#!/usr/bin/env node
/*
  Asterframe anti-slop scanner — dependency-free.
...
const isSkillDirectory = (path) => {
  const absolute = resolve(path);
  return absolute === skillRoot || (basename(absolute) === "asterframe" && basename(dirname(absolute)) === "skills");
};
...
function walk(dir, files = []) {
  if (isSkillDirectory(dir)) return files;
  let entries;
  try {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) {
      if (SKIP_DIRS.has(e.name) || isSkillDirectory(full)) continue;
      walk(full, files);
    } else if (e.isFile()) {
      const base = e.name;
...
console.log(`\n${bold("asterframe anti-slop")} — scanned ${files.length} files under ${escapeTerminal(root)}\n`);
if (groups.length === 0) {
