#!/usr/bin/env node
/**
 * Asterframe design hook — Cursor preToolUse write gate.
 *
...
function cursorBlockMessage(findings, filePath, config, cwd) {
  ...
  const blocked = rendered.replace(
    '[asterframe@1] Design hook findings requiring review',
    '[asterframe@1] Asterframe design hook blocked this write before it landed. Design hook findings requiring review',
  );
...
async function main() {
  if (truthy(process.env.ASTERFRAME_HOOK_DISABLED)) {
  ...
  if (denial.count > EDIT_COUNT_THRESHOLD) {
    const warning = `${message}\n\nThis is the ${denial.count}th repeated denial for the same file and finding signature, so Asterframe is allowing this write to avoid a loop. Reconsider the issue immediately after the tool runs.`;
    return allow({
      ...audit,
      findings: (findings || []).length,
...
main().catch((err) => {
  if (process.env.ASTERFRAME_HOOK_DEBUG) {
    process.stderr.write(`[asterframe-hook-before-edit] ${err}\n`);
  }
