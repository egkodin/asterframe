...
When the type carries the hierarchy on its own, hand off to `$asterframe polish` for the final pass.
...
See `references/live.md` for the full params contract.
...
The tactical selection procedure and the reflex-reject list live in [references/brand.md](brand.md) under **Font selection procedure** and **Reflex-reject list** (loaded for brand-register tasks). The rest of this section covers the adjacent knowledge: anti-reflex corrections, system font use, and pairing rules.
