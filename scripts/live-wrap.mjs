 ...
 *
 * Usage:
 *   npx asterframe wrap --id SESSION_ID --count N --query "hero-combined-left" [--file path]
 *
...
export async function wrapCli() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`Usage: asterframe wrap [options]
...
The agent should insert variant HTML at insertLine.`);
  ...
  const wrapperLines = isJsx ? [
    indent + '<div data-asterframe-variants="' + id + '" data-asterframe-variant-count="' + count + '" ' + styleContents + '>',
    indent + '  ' + commentSyntax.open + ' asterframe-variants-start ' + id + ' ' + commentSyntax.close,
    indent + '  ' + commentSyntax.open + ' Original ' + commentSyntax.close,
    indent + '  <div data-asterframe-variant="original">',
    reindentOriginal('    '),
    indent + '  </div>',
    indent + '  ' + commentSyntax.open + ' Variants: insert below this line ' + commentSyntax.close,
    indent + '  ' + commentSyntax.open + ' asterframe-variants-end ' + id + ' ' + commentSyntax.close,
    indent + '</div>',
  ] : [
    indent + commentSyntax.open + ' asterframe-variants-start ' + id + ' ' + commentSyntax.close,
    indent + '<div data-asterframe-variants="' + id + '" data-asterframe-variant-count="' + count + '" ' + styleContents + '>',
...
function detectStyleMode(filePath) {
  ...
  if (ext === '.astro') {
    return {
      ...
      styleTag: '<style is:inline data-asterframe-css="SESSION_ID">',
    };
  }
  return {
    mode: 'scoped',
    styleTag: '<style data-asterframe-css="SESSION_ID">',
  };
}

function buildCssSelectorPrefixExamples(styleMode, count) {
  if (styleMode !== 'astro-global-prefixed') return [];
  return Array.from({ length: count }, (_, i) => `[data-asterframe-variant="${i + 1}"]`);
}
...
function buildCssAuthoring(styleMode, count) {
  ...
  if (styleMode.mode === 'astro-global-prefixed') {
    return {
      ...
      rulePattern: '[data-asterframe-variant="N"] > .variant-class { ... }',
      selectorExamples: variantNumbers.map((n) => `[data-asterframe-variant="${n}"] > .variant-class`),
      requirements: [
        ...
        'Prefix every preview selector with the matching [data-asterframe-variant="N"] selector.',
  ...
  return {
    ...
    styleTag: styleMode.styleTag,
    strategy: 'scope-rule',
    rulePattern: '@scope ([data-asterframe-variant="N"]) { :scope > .variant-class { ... } }',
    selectorExamples: variantNumbers.map((n) => `@scope ([data-asterframe-variant="${n}"]) { :scope > .variant-class { ... } }`),
    requirements: [
      'Use @scope blocks keyed to each [data-asterframe-variant="N"] wrapper.',
    ...
    forbidden: [
      'Do not use global [data-asterframe-variant="N"] selector prefixes for this styleMode.',
...
function findElement(lines, query, tag = null) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (lines[i].includes('data-asterframe-variant')) continue;
