/**
 * Asterframe Live Variant Mode - Browser Script
 *
 * Injected into the user's page via <script src="http://localhost:PORT/live.js">.
 * The server prepends window.__ASTERFRAME_TOKEN__ and window.__ASTERFRAME_PORT__
 * before this code.
 *
...
(function () {
  ...
  if (window.__ASTERFRAME_LIVE_INIT__) return;
  window.__ASTERFRAME_LIVE_INIT__ = true;

  const TOKEN = window.__ASTERFRAME_TOKEN__;
  const PORT = window.__ASTERFRAME_PORT__;
  if (!TOKEN || !PORT) {
    window.__ASTERFRAME_LIVE_INIT__ = false; // reset so the real load can init
    return;
  }
  ...
  const PREFIX = 'asterframe-live';
  const PICK_CURSOR_CLASS = PREFIX + '-pick-cursor';
  const MANUAL_APPLY_STATE_TTL_MS = 15 * 60 * 1000;
  const sessionState = window.__ASTERFRAME_LIVE_SESSION__?.createLiveBrowserSessionState({
    prefix: PREFIX,
    storage: localStorage,
    idFactory: () => crypto.randomUUID().replace(/-/g, '').slice(0, 8),
  });
  if (!sessionState) {
    console.error('[asterframe] live-browser-session.js was not loaded. Live mode cannot start safely.');
    window.__ASTERFRAME_LIVE_INIT__ = false;
    return;
  }
  ...
  // window.__ASTERFRAME_VOCAB__ when it serves /live.js (same injection path as
  ...
  const VOCAB = Array.isArray(window.__ASTERFRAME_VOCAB__) ? window.__ASTERFRAME_VOCAB__ : [];
  const ICONS = {};
  ...
  const domHelpers = window.__ASTERFRAME_LIVE_DOM__?.createLiveBrowserDomHelpers({
    prefix: PREFIX,
    skipTags: SKIP_TAGS,
    document,
  });
  if (!domHelpers) {
    console.error('[asterframe] live-browser-dom.js was not loaded. Live mode cannot start safely.');
    window.__ASTERFRAME_LIVE_INIT__ = false;
    return;
  }
  const {
  ...
  window.__ASTERFRAME_LIVE_CHROME_CORE__ = {
    version: 1,
    adapter: window.__ASTERFRAME_LIVE_ADAPTER__ || 'dom',
  ...
  function showHighlight(el) {
    ...
    if (el.hasAttribute?.('data-asterframe-insert-placeholder')) return;
  ...
  function onAnnotDown(e) {
    ...
    const resizeEdge = e.target.closest?.('[data-asterframe-placeholder-resize]')?.dataset.asterframePlaceholderResize;
  ...
  function stripManualEditRuntimeState(root) {
    ...
    const nodes = [root, ...root.querySelectorAll('[data-asterframe-editable], [data-asterframe-original-text], [data-asterframe-text-wrap]')];
    for (const node of nodes) {
      const runtimeEditable = node.hasAttribute('data-asterframe-editable')
        || node.hasAttribute('data-asterframe-original-text');
      node.removeAttribute('data-asterframe-editable');
      node.removeAttribute('data-asterframe-original-text');
      node.removeAttribute('data-asterframe-text-wrap');
      if (runtimeEditable) {
  ...
  function selectionTagLabel(el) {
    if (!el) return '';
    if (el.hasAttribute?.('data-asterframe-insert-placeholder')) return 'slot';
  ...
  function applyPlaceholderSizingStyles(placeholder, sizing) {
    placeholder.dataset.asterframePlaceholderWidth = sizing.kind;
  ...
  function materializePlaceholderWidth(placeholder) {
    ...
    const kind = placeholder.dataset.asterframePlaceholderWidth;
    ...
    placeholder.dataset.asterframePlaceholderWidth = 'explicit';
  }
  ...
  function resolveBarAnchor() {
    ...
    if (currentSessionId && (state === 'GENERATING' || state === 'CYCLING')) {
      const wrapper = document.querySelector('[data-asterframe-variants="' + currentSessionId + '"]');
      if (wrapper) {
        const variantCount = wrapper.querySelectorAll('[data-asterframe-variant]:not([data-asterframe-variant="original"])').length;
  ...
  function isInsertGeneratingSession() {
    ...
    const wrapper = document.querySelector('[data-asterframe-variants="' + currentSessionId + '"]');
    return !!wrapper && wrapper.dataset.asterframeMode === 'insert';
  }
  ...
  function buildPlaceholderResizeHandles() {
    ...
    for (const spec of specs) {
      ...
      handle.dataset.asterframePlaceholderResize = spec.edge;
  ...
  function createInsertPlaceholder(anchor, position, layoutAxis) {
    ...
    placeholder.setAttribute('data-asterframe-insert-placeholder', 'true');
  ...
  function ensureConfigureInputStyle() {
    ...
    s.textContent =
      '@keyframes asterframe-configure-voice-pulse { 0%, 100% { opacity: 0.55; } 50% { opacity: 1; } }' +
      ...
      '#' + PREFIX + '-configure-voice[data-listening="true"] svg, #' + PREFIX + '-insert-voice[data-listening="true"] svg { animation: asterframe-configure-voice-pulse 1.1s ease-in-out infinite; }' +
  ...
  function buildSavingRow() {
    ...
    const spinner = el('div', {
      ...
      animation: 'asterframe-spin 0.6s linear infinite',
      flexShrink: '0',
    });
  ...
  function recoverEmptyCycling(reason) {
    ...
    try {
      console.warn('[asterframe] Refusing to render empty variant cycling state:', reason);
  ...
  // variant wrapper:
  //
  //   <div data-asterframe-variant="1"
  //        data-asterframe-params='[{"id":"density","kind":"steps",...}]'>
  //
  ...
  function getVisibleVariantEl() {
    ...
    return wrapper.querySelector('[data-asterframe-variant="' + visibleVariant + '"]');
  }

  function parseVariantParams(variantEl) {
    // Svelte component variants can't carry a `data-asterframe-params` attribute:
    ...
    if (!variantEl) return [];
    const raw = variantEl.getAttribute('data-asterframe-params');
    if (!raw) return [];
    try {
      const parsed = JSON.parse(raw);
      return Array.isArray(parsed) ? parsed : [];
    } catch (err) {
      console.warn('[asterframe] Invalid data-asterframe-params JSON:', err.message);
      return [];
    }
  }
  ...
  function wrapMixedContentTextNodes(rootEl) {
    ...
    if (hasText && hasElement) {
      for (const node of children) {
        if (node.nodeType === 3 && /\S/.test(node.nodeValue || '')) {
          ...
          wrap.dataset.asterframeTextWrap = 'true';
    ...
    for (const child of Array.from(rootEl.children)) {
      if (!child.dataset || !child.dataset.asterframeTextWrap) {
  ...
  function unwrapMixedContentTextNodes(rootEl) {
    if (!rootEl || rootEl.nodeType !== 1) return;
    const wraps = rootEl.querySelectorAll('[data-asterframe-text-wrap="true"]');
  ...
  function enableInlineEdit(targetEl) {
    ...
    for (const row of rows) {
      ...
      row.el.dataset.asterframeEditable = 'true';
      row.el.dataset.asterframeOriginalText = row.text;
  ...
  function disableInlineEdit(opts = {}) {
    for (const row of inlineEditRows) {
      ...
      delete row.el.dataset.asterframeEditable;
      delete row.el.dataset.asterframeOriginalText;
  ...
  function restoreInlineEditDrafts() {
    for (const row of inlineEditRows) {
      if (inlineEditDrafts.has(row.el)) {
        row.el.textContent = row.el.dataset.asterframeOriginalText;
      }
    }
  }
  ...
  function documentRefClassSuffix(el) {
    ...
    for (const cls of el.classList) {
      if (!cls || cls.indexOf('asterframe-') === 0) continue;
  ...
  function copyEditLeafContext(el, originalText, newText) {
    ...
    return {
      ...
      id: el.id || null,
      classes: el.classList ? [...el.classList].filter((cls) => cls.indexOf('asterframe-') !== 0) : [],
      originalText,
      newText,
      textContent: (el.textContent || '').slice(0, 500),
  ...
  function nearbyEditableTextsForManualEdit(rows, activeEl, originalText, newText) {
    ...
    for (const row of rows || []) {
      ...
      out.push({
        ...
        classes: row.el?.classList ? [...row.el.classList].filter((cls) => cls.indexOf('asterframe-') !== 0) : [],
        text,
      });
  ...
  async function applyEditing() {
    ...
    try {
      ...
      console.error('[asterframe] manual edit stash failed:', err);
  ...
  function ensureSpinKeyframes() {
    ...
    style.textContent = '@keyframes asterframe-spin { to { transform: rotate(360deg); } }';
    uiAppendStyle(style);
  }
  ...
  async function fetchPendingCount() {
    try {
      ...
      console.warn('[asterframe] failed to fetch pending count:', err);
    }
  }
  ...
  async function onPendingPillClick() {
    ...
    try {
      ...
      if (result.failed && result.failed.length > 0) {
        console.warn('[asterframe] some copy edits failed:', result.failed);
        ...
        if (n > 0) {
          ...
          console.warn('[asterframe] apply returned no verified edits:', result);
          showToast('No edits applied - see console', 4000);
        }
      }
    } catch (err) {
      console.error('[asterframe] commit failed:', err);
  ...
  async function onPendingTrashClick() {
    ...
    try {
      ...
      console.error('[asterframe] discard failed:', err);
  ...
  async function onPendingKeepFixingClick() {
    ...
    try {
      ...
      console.error('[asterframe] repair retry failed:', err);
  ...
  async function onPendingRollbackClick() {
    ...
    try {
      ...
      console.error('[asterframe] manual Apply rollback failed:', err);
  ...
  function restoreDiscardedManualEdits(entries) {
    ...
    if (failures > 0) {
      console.warn('[asterframe] skipped unsafe copy edit DOM restore for', failures, 'edit(s). Refresh to reset the page DOM.');
    }
    return failures;
  }
  ...
  function mixedTextWrapRestoreHint(el) {
    if (!el || !el.dataset || el.dataset.asterframeTextWrap !== 'true' || !el.parentElement) return null;
  ...
  function directMixedTextRestoreNodes(parent) {
    return Array.from(parent?.childNodes || []).filter((node) => {
      ...
      return node.nodeType === 1
        && node.dataset
        && node.dataset.asterframeTextWrap === 'true'
  ...
  function syncEditBadgeHitProxies() {
    ...
    for (const target of targets) {
      ...
      if (!proxy) {
        ...
        proxy.type = 'button';
        proxy.tabIndex = -1;
        proxy.dataset.asterframeEditBadgeProxy = 'true';
  ...
  function initEditBadge() {
    ...
    if (!uiGetById(PREFIX + '-edit-badge-focus-style')) {
      ...
      s.textContent =
        ...
        '[data-asterframe-editable="true"] { outline: none !important; box-shadow: none !important; }' +
        '[data-asterframe-editable="true"]:focus { outline: none !important; box-shadow: none !important; }' +
        '[data-asterframe-editable="true"]:focus-visible { outline: none !important; box-shadow: none !important; }';
      uiAppendStyle(s);
    }
  }
  ...
  async function showVariantInDOM(sessionId, num) {
    ...
    const wrapper = document.querySelector('[data-asterframe-variants="' + sessionId + '"]');
    if (!wrapper) return false;
    for (const child of wrapper.children) {
      const v = child.dataset ? child.dataset.asterframeVariant : null;
      if (!v) continue;
  ...
  function parseOriginalMarkupElement(originalMarkup) {
    ...
    const doc = parser.parseFromString('<div id="asterframe-anchor">' + originalMarkup + '</div>', 'text/html');
    return doc.getElementById('asterframe-anchor')?.firstElementChild || null;
  }
  ...
  function isUsableInjectionAnchor(el) {
    return !!el
      ...
      && !own(el)
      && !el.closest?.('[data-asterframe-variants]');
  }
  ...
  function waitForVariantAnchorAndRetry({ filePath, sessionId, srcWrapper, checkpointReason }) {
    ...
    const origContent = srcWrapper?.querySelector('[data-asterframe-variant="original"] > :first-child');
    ...
    pendingVariantAnchorRetryObserver = new MutationObserver(() => {
      ...
      const wrapperLanded = !!document.querySelector('[data-asterframe-variants="' + sessionId + '"]');
      if (!wrapperLanded) {
  ...
  function loadSvelteRuntime(runtimeModule) {
    const modulePath = runtimeModule || '/src/lib/asterframe/__runtime.js';
  ...
  // componentDir (keyed by variant number), because a `data-asterframe-params`
  ...
  async function applySvelteComponentVariantStyle(variantNum) {
    ...
    if (!scopedCss) return;
    const style = document.createElement('style');
    style.dataset.asterframeSvelteComponentStyle = sessionId;
    style.dataset.asterframeVariant = String(variantNum);
  ...
  function scopeCssToSveltePreview(css, sessionId) {
    const prefix = '[data-asterframe-variants="' + String(sessionId).replace(/"/g, '\\"') + '"] ';
  ...
  async function mountSvelteComponentVariant(variantNum) {
    ...
    try {
      ...
      console.error('[asterframe] Failed to mount Svelte variant ' + variantNum + ' for ' + sessionId + ':', err);
      return false;
    }
  }
  ...
  async function injectSvelteComponentsFromManifest(manifestPath, sessionId) {
    ...
    try {
      ...
      const existingWrapper = document.querySelector('[data-asterframe-variants="' + sessionId + '"]');
      ...
      if (!liveEl?.parentElement) {
        console.warn('[asterframe] Could not find original element in live DOM.');
        arrivedVariants = Number(manifest.count) || expectedVariants || 1;
        expectedVariants = arrivedVariants;
      ...
      wrapper.dataset.asterframeVariants = sessionId;
      wrapper.dataset.asterframeVariantCount = String(manifest.count || expectedVariants || 1);
      wrapper.dataset.asterframePreview = 'svelte-component';
      wrapper.style.display = 'contents';

      const mountTarget = document.createElement('div');
      mountTarget.dataset.asterframeComponentMount = sessionId;
      ...
      positionBar();
      saveSession();
      console.log('[asterframe] Mounted ' + arrivedVariants + ' Svelte component variants.');
    } catch (err) {
      console.error('[asterframe] Failed to mount Svelte component variants:', err);
  ...
  function abortSvelteComponentInjection(sessionId, message) {
    try {
      if (svelteComponentSession?.sessionId === sessionId) {
        ...
        const orphan = document.querySelector('[data-asterframe-variants="' + sessionId + '"]');
        if (orphan) orphan.remove();
      }
    } catch (err) {
      console.warn('[asterframe] Svelte component abort cleanup failed:', err);
    }
    hideShaderOverlay();
  ...
  function injectVariantsFromSource(filePath, sessionId) {
    ...
    fetch(url)
      ...
      .then(html => {
        ...
        const startMark = '<!-- asterframe-variants-start ' + sessionId + ' -->';
        const endMark = '<!-- asterframe-variants-end ' + sessionId + ' -->';
        ...
        srcWrapper = doc.querySelector('[data-asterframe-variants="' + sessionId + '"]');
        if (!srcWrapper) {
          console.warn('[asterframe] Variant wrapper not found in source file.');
          return;
        }
        ...
        const variants = wrapper.querySelectorAll('[data-asterframe-variant]:not([data-asterframe-variant="original"])');
        arrivedVariants = variants.length;
        expectedVariants = parseInt(wrapper.dataset.asterframeVariantCount || arrivedVariants);
        if (arrivedVariants <= 0) {
        ...
        positionBar();
        saveSession();
        console.log('[asterframe] Injected ' + arrivedVariants + ' variants from source file.');
      })
      .catch(err => {
        console.error('[asterframe] Failed to fetch source:', err);
  ...
  function updateSelectedElement() {
    ...
    if (svelteComponentSession?.sessionId === currentSessionId) {
      const anchor = resolveSvelteComponentAnchor();
      if (anchor && !anchor.__asterframeFrozenAnchor) selectedElement = anchor;
      return;
    }
    const wrapper = document.querySelector('[data-asterframe-variants="' + currentSessionId + '"]');
    if (!wrapper) return;
  ...
  function readVisibleVariantFromDOM(sessionId) {
    ...
    for (const variant of variants) {
      ...
      const idx = parseInt(variant.dataset.asterframeVariant || '0', 10);
      if (idx > 0) return idx;
    }
    return 0;
  }
  ...
  function pickVariantContent(wrapper, index) {
    if (!wrapper) return null;
    const variantDiv = wrapper.querySelector('[data-asterframe-variant="' + index + '"]');
  ...
  function startScrollLock(sessionId, initialTargetY) {
    ...
    scrollLockObserver = new MutationObserver((mutations) => {
      for (const m of mutations) {
        if (m.target?.closest?.('[data-asterframe-variants="' + sessionId + '"]')) {
          schedule('mutation-in-wrapper');
          return;
        }
        for (const n of m.addedNodes) {
          if (n.nodeType === 1 && (n.matches?.('[data-asterframe-variants="' + sessionId + '"]') || n.querySelector?.('[data-asterframe-variants="' + sessionId + '"]'))) {
  ...
  function startVariantObserver(sessionId) {
    ...
    const obs = new MutationObserver((mutations) => {
      ...
      // Only react to mutations that add nodes with data-asterframe-variant,
      // or mutations inside the variant wrapper. Ignore our own bar/UI changes.
      let dominated = false;
      for (const m of mutations) {
        if (m.target.closest?.('[data-asterframe-variants]')) { dominated = true; break; }
        for (const n of m.addedNodes) {
          if (n.nodeType !== 1) continue;
          // Direct hit: the added node itself is the wrapper or a variant.
          if (n.dataset?.asterframeVariants || n.dataset?.asterframeVariant) {
          ...
          if (n.querySelector?.('[data-asterframe-variants],[data-asterframe-variant]')) {
            dominated = true; break;
          }
        }
        if (dominated) break;
      ...
      if (selectedElement && !document.body.contains(selectedElement)) {
        const isInsert = wrapper.dataset.asterframeMode === 'insert';
        if (isInsert) {
      ...
      const expected = parseInt(wrapper.dataset.asterframeVariantCount || '0');
      ...
      if (arrivedVariants >= expectedVariants && expectedVariants > 0) {
        ...
        recoveryWaitingForAnchor = false;
        hideShaderOverlay();
        if (wrapper.dataset.asterframeMode === 'insert') finalizeInsertSession();
        updateSelectedElement();
        showOrUpdateCyclingBar();
        disableInlineEdit();
        refreshParamsPanel();
  ...
  function startScrollTracking() {
    function tick() {
      if (state === 'CONFIGURING' || state === 'GENERATING' || state === 'CYCLING') {
        ...
        if (hiTarget && !hiTarget.hasAttribute?.('data-asterframe-insert-placeholder')) {
  ...
  function connectSSE() {
    ...
    evtSource.onmessage = (e) => {
      ...
      switch (msg.type) {
        case 'connected':
          ...
          if (!hasProjectContext) showToast('No PRODUCT.md found. Variants will be brand-agnostic. Run /asterframe init to generate one.', 7000);
          console.log('[asterframe] Live mode connected.');
        ...
        case 'error':
          ...
          if (maybeCompleteSteer(msg)) break;
          console.error('[asterframe] Error:', msg.message);
    ...
    evtSource.onerror = () => {
      ...
      if (sseRetries <= SSE_MAX_RETRIES) {
        console.log('[asterframe] SSE connection lost. Retry ' + sseRetries + '/' + SSE_MAX_RETRIES + '...');
        return; // EventSource auto-reconnects
      }
      // Server is gone. Clean up gracefully.
      console.log('[asterframe] Live server unreachable. Cleaning up UI.');
      evtSource.close();
      evtSource = null;
      handleServerLost();
    };
  ...
  function sendEvent(msg, opts) {
    ...
    function handleFailure(err) {
      if (opts && opts.throwOnError) {
        console.error('[asterframe] Failed to send event:', err);
        throw err;
      }
      console.debug('[asterframe] Dropped optional live event:', err);
      return null;
    }
  ...
  function handleKeyDown(e) {
    ...
    if (e.target.isContentEditable && inlineEditRows.some((r) => r.el === e.target)) {
      ...
      e.preventDefault();
      e.stopPropagation();
      const original = e.target.dataset.asterframeOriginalText;
  ...
  function shouldUseAncestorCropShaderProxy(el) {
    ...
    const adapter = String(window.__ASTERFRAME_LIVE_ADAPTER__ || '').toLowerCase();
    if (adapter === 'svelte' || adapter === 'sveltekit') return true;
    if (currentPreviewMode === 'svelte-component' || svelteComponentSession) return true;
    const wrapper = el?.closest?.('[data-asterframe-variants]');
    return wrapper?.dataset?.asterframePreview === 'svelte-component';
  }
    ...
    try {
      ...
      if (shouldUseAncestorCropShaderProxy(el)) {
        try {
          ...
          console.warn('[asterframe] Svelte ancestor crop capture failed, falling back to element capture:', err);
        }
      }
  ...
  async function captureAndEmit(el, basePayload, snapshot, rect) {
    ...
    try {
      ...
      console.warn('[asterframe] capture failed, proceeding without screenshot:', err);
    }
    ...
    if (blob && hasAnnotations) {
      try {
        ...
        if (uploadRes.ok) {
          ...
          console.warn('[asterframe] annotation upload failed:', uploadRes.status);
        }
      } catch (err) {
        console.warn('[asterframe] annotation upload failed:', err);
      }
    }
...
void main() {
  ...
  async function showShaderOverlay(el, blob, rect, paper) {
    ...
    try {
      ...
      console.warn('[asterframe] shader setup failed:', err);
      canvas.remove();
      return;
    }
    ...
    try {
      ...
      console.warn('[asterframe] shader bitmap decode failed:', err);
  ...
  async function handleAccept() {
    ...
    const acceptWrapper = document.querySelector('[data-asterframe-variants="' + currentSessionId + '"]');
    ...
    const acceptedIsSvelteComponent = svelteComponentSession?.sessionId === acceptedSessionId
      || acceptWrapper?.dataset?.asterframePreview === 'svelte-component';
  ...
  function snapshotAcceptedVariantDom(sessionId, variantId) {
    const wrapper = document.querySelector('[data-asterframe-variants="' + sessionId + '"]');
    const accepted = wrapper?.querySelector?.('[data-asterframe-variant="' + variantId + '"]');
  ...
  function acceptedDomAlreadyClean(pending) {
    ...
    return matches.length > 0
      && matches.every((el) => !el.closest('[data-asterframe-variants],[data-asterframe-variant],[data-asterframe-carbonize]'));
  }
  ...
  function findAcceptedRuntimeWrappers(sessionId) {
    if (!sessionId) return [];
    return [...new Set([
      ...document.querySelectorAll('[data-asterframe-variants="' + sessionId + '"]'),
      ...document.querySelectorAll('[data-asterframe-carbonize="' + sessionId + '"]'),
    ])];
  }
  ...
  function reloadAfterMissingAcceptedDom(pending) {
    if (acceptedDomAlreadyClean(pending)) return;
    if (pending?.id && document.querySelector('[data-asterframe-variants="' + pending.id + '"]')) return;
    location.reload();
  }
  ...
  function commitAcceptedVariantToDom(sessionId, variantId) {
    ...
    const style = wrapper.querySelector('style[data-asterframe-css]');
    if (style && !document.querySelector('style[data-asterframe-accepted-css="' + sessionId + '"]')) {
      const promotedStyle = style.cloneNode(true);
      promotedStyle.setAttribute('data-asterframe-accepted-css', sessionId);
  ...
  function restoreFromActiveSessions(activeSessions, reason) {
    const wrapper = document.querySelector('[data-asterframe-variants]');
    if (wrapper && wrapper.dataset.asterframePreview !== 'svelte-component') return false;
  ...
  function cleanup() {
    ...
    if (svelteComponentSession?.sessionId === cleanupSessionId) {
      ...
      const wrapper = document.querySelector('[data-asterframe-variants="' + cleanupSessionId + '"]');
      if (wrapper) wrapper.style.display = 'none';
      setTimeout(function() {
        if (!cleanupSessionId) return;
        const lateWrapper = document.querySelector('[data-asterframe-variants="' + cleanupSessionId + '"]');
        if (!lateWrapper) return;
        const orig = lateWrapper.querySelector('[data-asterframe-variant="original"]');
        if (orig) {
  ...
  // If a [data-asterframe-variants] wrapper exists in the DOM, the agent wrote
  ...
  function resumeSession() {
    ...
    const sessionId = wrapper.dataset.asterframeVariants;
    ...
    // wrapper holds a single mount target, not [data-asterframe-variant] nodes,
    ...
    if (wrapper.dataset.asterframePreview === 'svelte-component'
    ...
    if (wrapper.dataset.asterframePreview === 'svelte-component') {
      ...
      currentSessionId = sessionId;
      expectedVariants = Number(wrapper.dataset.asterframeVariantCount)
    ...
    currentSessionId = sessionId;
    expectedVariants = parseInt(wrapper.dataset.asterframeVariantCount || '0');
    const variants = wrapper.querySelectorAll('[data-asterframe-variant]:not([data-asterframe-variant="original"])');
    arrivedVariants = variants.length;
      ...
      if (shaderTarget) {
        (async () => {
          try {
            ...
            console.warn('[asterframe] shader resume failed:', err);
          }
        })();
      }
    }
  ...
  const PICK_PREFS_KEY = 'asterframe-live-pick';
  const INTERACTION_PREFS_KEY = 'asterframe-live-interaction';
  ...
  function detectPageTheme() {
    try {
      // Dev override: set localStorage 'asterframe-dev-theme' to 'light' or
      // 'dark' to preview the opposite palette without actually changing the
      // page bg. Used for screenshots and theme QA.
      const override = localStorage.getItem('asterframe-dev-theme');
  ...
  function attachSteerFocusGuard() {
    if (window.__ASTERFRAME_STEER_FOCUS_GUARD__) return;
    window.__ASTERFRAME_STEER_FOCUS_GUARD__ = true;
  ...
  function steerFocusDebugEnabled() {
    try { return localStorage.getItem('asterframe-steer-debug') === '1'; } catch { return false; }
  }

  function steerFocusLog(reason, extra) {
    if (!steerFocusDebugEnabled()) return;
    console.log('[asterframe.steer]', reason, {
      state,
      pickActive,
  ...
  function attachSteerFocusDebug() {
    if (!steerFocusDebugEnabled()) return;
    if (window.__ASTERFRAME_STEER_FOCUS_DEBUG__) return;
    window.__ASTERFRAME_STEER_FOCUS_DEBUG__ = true;
  ...
  function buildSteerProcessingDots() {
    ...
    for (let i = 0; i < 3; i++) {
      wrap.appendChild(el('span', {
        ...
        animation: 'asterframe-steer-dot 1.05s ease-in-out ' + (i * 0.14) + 's infinite',
      }));
    }
    return wrap;
  }
  ...
  function startVoice(ctx) {
    ...
    rec.onerror = (event) => {
      ...
      console.warn('[asterframe.voice] recognition error:', code);
    ...
    try {
      ...
      console.warn('[asterframe.voice] start failed:', err);
      stopVoice({
        suppressSubmit: true,
  ...
  function initPageChat(parent, P) {
    ...
    if (!uiGetById(PREFIX + '-page-chat-style')) {
      ...
      s.textContent =
        '@keyframes asterframe-steer-dot { 0%, 70%, 100% { opacity: 0.28; transform: scale(0.82); } 35% { opacity: 1; transform: scale(1); } }' +
        '@keyframes asterframe-steer-processing { 0%, 100% { border-color: oklch(70% 0.12 188 / 0.28); box-shadow: 0 0 0 0 oklch(70% 0.12 188 / 0); } 50% { border-color: oklch(82% 0.07 188 / 0.55); box-shadow: 0 0 14px oklch(70% 0.12 188 / 0.18); } }' +
        '@keyframes asterframe-voice-pulse { 0%, 100% { opacity: 0.55; } 50% { opacity: 1; } }' +
        '#' + PREFIX + '-page-chat[data-processing="true"] { animation: asterframe-steer-processing 1.6s ease-in-out infinite; }' +
        ...
        '#' + PREFIX + '-page-chat-voice[data-listening="true"] svg { animation: asterframe-voice-pulse 1.1s ease-in-out infinite; }' +
  ...
  // Asterframe mark - same paths as site/components/Header.astro + favicon.svg.
  ...
  function syncAgentPollingUi(connected) {
    ...
    globalBarBrandEl.setAttribute('aria-label', connected
      ? 'Asterframe live mode'
      : 'Asterframe live mode - agent not polling');
  ...
  function initGlobalBar() {
    ...
    if (!uiGetById(PREFIX + '-bar-focus-style')) {
      ...
      s.textContent =
        ...
        '}' +
        '@keyframes asterframe-agent-dot { 0%, 100% { opacity: 0.45; transform: scale(0.9); } 50% { opacity: 1; transform: scale(1); } }' +
        '#' + PREFIX + '-global-bar-brand[data-agent-connected="false"] [data-agent-dot] { animation: asterframe-agent-dot 1.4s ease-in-out infinite; }' +
    ...
    // Brand mark - kinpaku Asterframe icon (site header / favicon paths).
    const brand = el('span', {
      display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
    ...
    brand.setAttribute('aria-label', 'Asterframe live mode - agent not polling');
  ...
  function updateGlobalBarState() {
    ...
    document.querySelectorAll('.asterframe-overlay').forEach(o => {
      o.style.pointerEvents = (pickActive || insertActive) ? 'none' : '';
    });
    syncPageInteractionCursor();
  }

  let detectReady = false; // true once detect script posts 'asterframe-ready'
  ...
  function toggleDetect() {
    ...
    if (detectActive) {
      ...
      window.postMessage({ source: 'asterframe-command', action: 'remove' }, '*');
  ...
  function loadDetectScript() {
    ...
    s.dataset.asterframeExtension = 'true';
  ...
  function onDetectMessage(e) {
    ...
    if (e.data.source === 'asterframe-ready') {
      detectReady = true;
    ...
    if (e.data.source === 'asterframe-results') {
  ...
  function teardown() {
    ...
    window.__ASTERFRAME_LIVE_INIT__ = false;
    console.log('[asterframe] Live mode exited.');
  }

  //
  // Design System Panel - visualizes the project's .asterframe/design.json sidecar
  //

  const DESIGN_PREFS_KEY = 'asterframe-live-design-panel';
  ...
  let designState = {
    ...
    sidecar: null,          // .asterframe/design.json v2 payload (extensions + components + narrative)
    hasMd: false,
    hasSidecar: false,
  ...
  // Neutral panel palette - deliberately NOT Asterframe-branded. The panel is
  // a viewer of the project's design system, not an Asterframe surface.
  const DP = {
  ...
  function renderDesignBody() {
    ...
    if (designState.present === false) {
      ...
      empty.innerHTML = `<strong>No DESIGN.md yet</strong>Create one by running <code>/asterframe document</code> in your terminal, then re-open this panel.`;
  ...
  function renderStaleHint() {
    ...
    box.innerHTML = `
      ...
      <span class="stale-text"><strong>DESIGN.md is newer than .asterframe/design.json.</strong> Run <code>/asterframe document</code> to refresh the sidecar.</span>
    `;
    return box;
  }
  ...
  function renderParsedMdCta() {
    ...
    box.innerHTML = `<strong>Basic view</strong>This panel reads the tokens in your <code>DESIGN.md</code> frontmatter. Running <code>/asterframe document</code> also generates a <code>.asterframe/design.json</code> sidecar with your project's actual component snippets (button, input, nav) and tonal ramps, rendered live below the tokens.`;
    return box;
  }
  ...
  function cssSafe(v) {
    ...
    // .asterframe/design.json values rendered into inline style strings.
  ...
  function init() {
    ...
    if (!resumeSession()) {
      console.log('[asterframe] Live variant mode ready. Hover over elements to pick one.');
      ...
      const scout = new MutationObserver(() => {
        ...
        scout.disconnect();
        if (resumeSession()) {
          console.log('[asterframe] Resumed deferred session ' + currentSessionId + ' (post-hydration).');
        }
      });
      scout.observe(document.body, { childList: true, subtree: true });
    } else {
      console.log('[asterframe] Resumed active variant session ' + currentSessionId + ' (' + arrivedVariants + '/' + expectedVariants + ' variants).');
    }
