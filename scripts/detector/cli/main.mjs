...
} from '../../lib/asterframe-config.mjs';
import {
  HTML_EXTENSIONS,
  buildImportGraph,
...
function printUsage() {
  console.log(`Usage: asterframe detect [options] [file-or-dir...]
...
Options:
  ...
  --no-design-system  Do not load local DESIGN.md / .asterframe/design.json context
  --help              Show this help message

Project config:
  Respects .asterframe/config.json and .asterframe/config.local.json detector
...
Detection modes:
  ...
  URLs           Inspect with Obscura; this CLI intentionally scans local files only

Examples:
  asterframe detect src/
  asterframe detect index.html
  asterframe detect --json .
  asterframe detect --no-config src/`);
}
    ...
    try {
      for (const target of paths) {
        if (/^https?:\/\//i.test(target)) {
          process.stderr.write('Error: inspect URL targets with Obscura, then scan the corresponding local source with Asterframe.\n');
          process.exit(1);
        }
            ...
            if (fwConfig) {
              ...
              if (probe.listening && probe.matched) {
                process.stderr.write(
                  ...
                  `  npx asterframe detect http://localhost:${fwConfig.port}\n\n`
                );
