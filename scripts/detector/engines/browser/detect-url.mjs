...
async function runVisualContrastFallback(page, serializedGroups, options, profile, target) {
  ...
  if (options?.visualContrastBrowser !== false) {
    const browserFindings = await profileFindingsAsync(profile, {
      ...
      browserAnalyses = await page.evaluate(async ({ maxCandidates, scrollOffscreen }) => {
        if (typeof window.asterframeAnalyzeVisualContrast !== 'function') return [];
        return window.asterframeAnalyzeVisualContrast({ maxCandidates, scrollOffscreen });
  ...
  if (candidates.length === 0) {
    candidates = await profileStepAsync(profile, {
      ...
      if (typeof window.asterframeCollectVisualContrastCandidates !== 'function') return [];
      return window.asterframeCollectVisualContrastCandidates({ maxCandidates });
    }, { maxCandidates }));
  }
...
// ---------------------------------------------------------------------------
// External browser detection (for URLs)
// ---------------------------------------------------------------------------
...
async function detectUrl(url, options = {}) {
  ...
  if (!externalBrowser) {
    throw new Error('URL scanning requires an externally supplied Obscura browser session; use the file-based detector from the CLI.');
  }
  ...
  try {
    ...
    results = await profileFindingsAsync(profile, {
      ...
      serializedGroups = await page.evaluate(() => {
        if (!window.asterframeDetect) return [];
        return window.asterframeDetect({ decorate: false, serialize: true });
      });
...
async function createBrowserDetector(options = {}) {
  ...
  if (!browser) {
    throw new Error('Pass an externally supplied Obscura browser session. Asterframe never launches Chromium.');
  }
  const defaults = {
  ...
  return {
    ...
    async close() {
      // The caller owns the Obscura session.
    },
  };
}
