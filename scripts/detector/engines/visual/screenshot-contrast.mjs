...
async function captureVisualContrastCandidate(page, candidate, viewport) {
  ...
  const token = `asterframe-contrast-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  const applied = await page.evaluate(({ selector, token, backgroundClipText }) => {
    ...
    if (!el) return false;
    let style = document.getElementById('asterframe-visual-contrast-hide-style');
    if (!style) {
      style = document.createElement('style');
      style.id = 'asterframe-visual-contrast-hide-style';
      style.textContent = [
        '[data-asterframe-visual-contrast-target] {',
        '  color: transparent !important;',
        '  -webkit-text-fill-color: transparent !important;',
        '  text-shadow: none !important;',
        '}',
        '[data-asterframe-visual-contrast-target][data-asterframe-bgclip-text="true"] {',
        '  background-image: none !important;',
        '}',
      ].join('\n');
      document.head.appendChild(style);
    }
    el.setAttribute('data-asterframe-visual-contrast-target', token);
    if (backgroundClipText) el.setAttribute('data-asterframe-bgclip-text', 'true');
    return true;
  }, {
  ...
  try {
    ...
    await page.evaluate(({ selector }) => {
      try {
        ...
        if (el) {
          el.removeAttribute('data-asterframe-visual-contrast-target');
          el.removeAttribute('data-asterframe-bgclip-text');
        }
      } catch {
