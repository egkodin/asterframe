 ...
 *
 * `pin audit` creates a lightweight /audit skill that redirects to /asterframe audit.
...
const PIN_MARKER = '<!-- asterframe-pinned-skill -->';
...
/**
 * Find harness skill directories that have an asterframe skill installed.
 */
function findHarnessDirs(projectRoot) {
  const dirs = [];
  for (const harness of HARNESS_DIRS) {
    const skillsDir = join(projectRoot, harness, 'skills');
    // Only pin in harness dirs that already have asterframe installed
    const asterframeDir = join(skillsDir, 'asterframe');
    if (existsSync(asterframeDir) || existsSync(join(skillsDir, 'i-asterframe'))) {
      dirs.push(skillsDir);
    }
  }
  return dirs;
...
function generatePinnedSkill(command, metadata) {
  const desc = metadata[command]?.description || `Shortcut for /asterframe ${command}.`;
...
This is a pinned shortcut for \`{{command_prefix}}asterframe ${command}\`.

Invoke {{command_prefix}}asterframe ${command}, passing along any arguments provided here, and follow its instructions.
`;
}
...
function pin(command, projectRoot) {
  ...
  if (harnessDirs.length === 0) {
    console.log('No harness directories with asterframe installed found.');
    return false;
  }
...
function unpin(command, projectRoot) {
  ...
  if (removed > 0) {
    ...
    console.log(`Use /asterframe ${command} to access it.`);
  } else {
