#!/usr/bin/env node
/**
 * Asterframe design hook — PostToolUse entry point.
 *
...
async function main() {
  ...
  process.env.ASTERFRAME_HOOK_DEPTH = process.env.ASTERFRAME_HOOK_DEPTH || '1';
...
main().catch((err) => {
  ...
  if (process.env.ASTERFRAME_HOOK_DEBUG) {
    process.stderr.write(`[asterframe-hook] ${err}\n`);
  }
  process.exit(0);
});
