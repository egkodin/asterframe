 ...
 *   3. $ASTERFRAME_CONTEXT_DIR (absolute or cwd-relative) — power-user
...
// can offer `npx asterframe update`. Everything here is best-effort and
// silent on failure: a network problem, sandbox, or missing cache must never
// block context output or print an error.

const UPDATE_HOST = (process.env.ASTERFRAME_UPDATE_HOST || 'https://asterframe.style').replace(/\/$/, '');
const UPDATE_CACHE_PATH =
  process.env.ASTERFRAME_UPDATE_CACHE || path.join(os.homedir(), '.asterframe', 'update-check.json');
...
export function resolveContextDir(cwd = process.cwd()) {
  ...
  const envDir = process.env.ASTERFRAME_CONTEXT_DIR;
...
function buildUpdateDirective(localVersion, latestVersion) {
  return (
    `UPDATE_AVAILABLE: A newer Asterframe skill is available ` +
    `(installed v${localVersion}, latest v${latestVersion}). ` +
    `Before continuing, ask the user once: "A newer Asterframe (v${latestVersion}) is available. ` +
    `Update now? It runs \`npx asterframe update\`." ` +
    `If they agree, run \`npx asterframe update\` (the update applies to the next session, not this one). ` +
 ...
 * the agent never nags. Opt out entirely with ASTERFRAME_NO_UPDATE_CHECK=1.
 */
...
function updateCheckDisabledByConfig(cwd = process.cwd()) {
  ...
  for (const name of ['config.json', 'config.local.json']) {
    try {
      const raw = JSON.parse(fs.readFileSync(path.join(cwd, '.asterframe', name), 'utf-8'));
...
async function computeUpdateDirective(now = Date.now()) {
  try {
    if (process.env.ASTERFRAME_NO_UPDATE_CHECK) return null;
...
async function cli() {
  ...
  const next = register
    ? `NEXT STEP: This project's register is \`${register}\`. You MUST now read \`references/${register}.md\` before producing any design output.`
    : `NEXT STEP: You MUST now read the matching register reference (\`references/brand.md\` or \`references/product.md\`) before producing any design output. Pick based on PRODUCT.md above.`;
  parts.push(next);
  if (updateDirective) parts.push(updateDirective);
  process.stdout.write(parts.join('\n\n---\n\n') + '\n');
}
